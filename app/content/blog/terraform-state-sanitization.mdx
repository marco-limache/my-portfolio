...
With modern Terraform features and providers, we can utilize `write_only` attributes. When a resource defines an attribute as write-only, Terraform sends the value to the provider but _does not_ persist it in the state file.
